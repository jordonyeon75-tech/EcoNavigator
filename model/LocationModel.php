...
// Delete Rating
    ...
    public function deleteBookmarksBulk($userId, $bookmarkIds) {
        ...
        // Create the right amount of placeholders (e.g., ?,?,?) for the IDs
        $placeholders = implode(',', array_fill(0, count($bookmarkIds), '?'));

        // Delete all selected bookmarks in one fast query
        $stmt = $this->pdo->prepare("DELETE FROM bookmark WHERE user_id = ? AND id IN ($placeholders)");

        // Combine the userId and the bookmarkIds into one array to execute
        $params = array_merge([$userId], $bookmarkIds);
